...
public class Teleport extends Teleporter {
	...
	public static void teleportToDimension(EntityPlayer player, int dimension, double x, double y, double z, float yaw, float pitch) {
		...
		WorldServer worldServer = server.getWorld(type);
		...
		worldServer.getServer().getPlayerList().changePlayerDimension(entityPlayerMP, type, new Teleport(worldServer, x, y, z, yaw, pitch));
		player.setPositionAndUpdate(x, y, z);
		player.setPositionAndRotation(x, y, z, yaw, pitch);
	}
