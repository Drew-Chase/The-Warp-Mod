...
public class Warps {
	...
	public List<Warp> getWarps(EntityPlayer player) {
	...
	public List<Warp> getWarps(String player) {
		List<Warp> value = new ArrayList<Warp>();
		for (Warp warp : warps) {
			if (warp.getPlayer().getDisplayNameString().equalsIgnoreCase(player))
				value.add(warp);
		}
		return value;
	}
	...
	public void empty(EntityPlayer player, List<Warp> list) {
		for (Warp warp : list) {
			if (warp.getPlayer().equals(player)) {
				removeWarp(warp);
			}
